...
class PerfBasic(Test):
    ...
    def test_config_PMU_sysfs(self):
        ...
        # allow_output_check='combined' helps in combining stdout, stderr
        output = process.system_output("cat %smmcr*" % sysfs_file,
    ...
    def _verify_lscpu_sysfs(self, filename, value_to_verify):
        output = process.system_output("cat %s" % filename, sudo=True,
    ...
    def _check_count(self, event_type):
        ...
        sys_fs_events = os.listdir(base_dir + event_type + '/events/')
    ...
    # TBD: collect only cpu, hv_24x7, imc
